    ...
    */
    ...
    FreqDomainConvolver::FreqDomainConvolver(const std::vector<float>& h,
        ...
        : fftOrder(fftOrder),
          ...
          N((int) h.size()),
          fft(fftOrder),
          Hspec(K), Xspec(K), Yspec(K), timeC(K),
          timeK(K, 0.0f)
    {
        jassert(N > 0);
    ...
    std::vector<float> FreqDomainConvolver::processBlock(const float* x, int numSamples)
    {
        ...
        // newOverlap[t] = overlap[t + head] + (timeC[head + t].real()/K if t < tail else 0)
        std::vector<float> newOverlap((size_t)(N - 1), 0.0f);

        // Copy shifted remainder of previous overlap that wasn't emmited due to B being smaller than N - 1
    ...
    std::vector<float> FreqDomainConvolver::processBlock(const std::vector<float>& x)
    {
